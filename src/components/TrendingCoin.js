...
const TrendingCoin = ({ data }) => {
  ...
  return (
    <div
      ...
      className="md:w-[40%] w-[80%] bg-gray-200 mb-12 last:mb-0 rounded-lg p-4 relative cursor-pointer hover:bg-gray-100 hover:bg-opacity-40"
    >
      {data ? (
        <>
          <h3 className="md:text-base text-[12px] flex items-center my-0.5">
          ...
          <img
            src={data?.large}
            alt={data?.name}
            className="w-[25%] h-auto absolute top-1/4 -right-12 -translate-y-2/4 rounded-full"
          />
        </>
      ) : (
